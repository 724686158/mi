...
复杂的实际情况要求系统不能只有一种解决方案。

所以在设计智能爬虫时采用了精准爬取与模糊爬取并存的解决方案。精准爬取是指用人工方法定位到实际所需的数据，用工作量换取覆盖率，凡是被覆盖的网站，均可具有很高的准确性。模糊爬取是指提高对未知情况的适应能力，在算法层面上作出突破，借此提高准确率。这两种方法，都有相应的极限。对于精准爬取，对各大门户网站的人工分析，能用较少的人工成本，换取较大的覆盖率，但如果一味得通过人工编写爬虫业务来提高覆盖率，很快就会进入一个极为低效的领域。对于模糊爬取，具有一定水平的算法与技术是易于掌握的，比如优秀的开源项目如jieba、Goose，数据挖掘方面成熟的算法如支持向量机、朴素贝叶斯等，常用的页面分析算法如加分算法，锚链接分析。这些算法与技术能解决一部分的需求，但如果想在此基础上在作出突破，就变得相当困难。

在此基础上，工作走向了三个方向：

* 增加人工劳动，获得更高的覆盖率，这一方向对于电商网站有很好的效果。

* 优化

#### 任务信息自动加载

智能爬虫本身不包含任务信息，在创建其容器时也不需要传递任何参数。
...
### Bloom Filter
 ...
  使用Bloom Filter数据结构及有关算法算法完成url对去重过滤。
  
