...
def calc_score(url):
    ...
    # 文章关键词
    PAT = ('news', 'article', 'story', 'content', 'xinwen', 'detail', 'view', 'edition', 'essay', '/a/', 'p=', 'id=', 'archives', '/a-')
    for i in PAT:
        if i in url:
            score += 2
    # 类似 news_xxxx_110
    for i in PAT:
        a = re.findall(i + r'[-_/]?[a-zA-Z0-9]+[-_/]\d+', url)
        if len(a) > 0:
            score += 2
    # 日期，年月
    ...
    a = re.findall(r'/[a-zA-Z0-9]{4,}/\d{3,}[-/_]?\d?.s?htm', url)
    if len(a) > 0:
        score += 2
        # 如果上面这种典型后缀串紧跟在域名段之后（因为域名段替换掉，故形如www./）
        a = re.findall(r'./[a-zA-Z0-9]{4,}/\d{3,}[-/_]?\d?.s?htm', url)
        if len(a) > 0:
            score += 1
