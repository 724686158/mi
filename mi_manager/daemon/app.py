...
tasks = []
def test():
    mission1 = Mission('任务1', '{"start_time": ' + str(time.time() + 4) + ', "end_time":' + str(time.time() + 20) + ', "submission_list":[], "resource_dic":{}, "weight":0.8, "state": "START"}')
    ...
    while True:
        ...
        for mission in missions:
            ...
            if mission.state == 'READY':
                submissions_name_list = mission.get_submission_list()
                for submission_name in submissions_name_list:
                    submissions.append(Submisson(submission_name, data_service.get_submission(submission_name), mission.get_name(), mission.get_resource_dic(), mission.weight))
            ...
            # 对完成的任务, 從隊列中去除子任務
                ...
                submissions_name_list = mission.get_submission_list()
                for submission_name in submissions_name_list:
                    submissions.remove(Submisson(submission_name, data_service.get_submission(submission_name), mission.get_name(), mission.get_resource_dic(), mission.weight))
            # 对未到执行时间的任务不做处理
        ...
        for task in tasks_ready:
            # 根据task新建对应的json文件
            jsonfile_name = data_service.gen_json(task)
            # 用json文件创建新容器
            data_service.run_new_container('jsonfile_name')
